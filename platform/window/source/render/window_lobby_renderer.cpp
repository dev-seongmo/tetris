...
WindowLobbyRenderer::WindowLobbyRenderer(const InputWindowRenderer& input_render,
...
void WindowLobbyRenderer::render_input_window(int x, int y, const string& str)
{
    input_window_renderer.render_input_window({x, y}, str);
}

void WindowLobbyRenderer::render_big_text(int x, int y, const string& str)
{
    text_renderer.print_big_string({x, y}, str);
}

void WindowLobbyRenderer::render_big_text(int x, int y, const string& str, Color color)
{
    text_renderer.print_big_string({x, y}, str, color);
}

void WindowLobbyRenderer::render_small_text(int x, int y, const string& str, Color color)
{
    text_renderer.print_small_string({x, y}, str, color);
}

void WindowLobbyRenderer::render_set_nickname(const string& cur_nickname)
{
    render_clear();
...
void WindowLobbyRenderer::render_entrance()
{
    render_clear();
    render_big_text(22, 7, "ENTRANCE", Color::ORANGE);
}

void WindowLobbyRenderer::render_entrance_choice(Entrance entrance)
{
    const int X = 25, Y = 18;
    string text;
    Color text_color;
...
void WindowLobbyRenderer::render_create_room()
{
    render_clear();
...
void WindowLobbyRenderer::render_room(const string& room_name, const string& host_name, bool is_server)
{
    render_clear();
...
void WindowLobbyRenderer::render_room_clients(unordered_map<string, string>& client_ip_address)
{
    int X = 27, Y = 24, dy = 0;
...
void WindowLobbyRenderer::render_lobby()
{
    render_clear();
...
void WindowLobbyRenderer::render_lobby_rooms(vector<pair<string, string>>& rooms, int select)
{
    int X = 27, Y = 20, dy = 0;
